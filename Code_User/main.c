...
u8 left_thr = 0; //×ó´«¸ÐÆ÷ãÐÖµ

u8 middle_thr = 0;//ÖÐ¼ä´«¸ÐÆ÷Öµ

u8 right_thr = 0;//ÓÒ´«¸ÐÆ÷ãÐÖµ


u8 auto_mode_run_flag = 0;//×Ô¶¯Ä£Ê½ÏÂÆô¶¯±êÖ¾Î»

char run_str[16] = {0};//ÔËÐÐÖ¸Ê¾×Ö·û´®
        ...
        if(menu_auto.lock1 == 0) {
            ...
            OLED_ShowString(0,6,run_str,16);
				...
				if( F_100MS == 1 )
				{
						...
						// 	oledÏÔÊ¾
						//-----------------------------------------------------
						OLED_ShowNum(0,2,LEFT_TRA_ADC,3,16);//×ó´«¸ÐÆ÷Öµ
						OLED_ShowNum(40,2,CENTER_TRA_ADC,3,16);//ÖÐ´«¸ÐÆ÷Öµ
						OLED_ShowNum(80,2,RIGHT_TRA_ADC,3,16);//ÓÒ´«¸ÐÆ÷Öµ
					
						OLED_ShowNum(0,4,left_thr,3,16);//×ó´«¸ÐÆ÷ãÐÖµ
						OLED_ShowNum(80,4,right_thr,3,16);//ÓÒ´«¸ÐÆ÷ãÐÖµ
    ...
    case DEFAULT_MODE:
        ...
        case 3://ÏÂ
            ...
            setMenuReDisplay(&menu_auto);
						auto_mode_run_flag = 0;//Ä¬ÈÏÎªµÈ´ýÉèÖÃ
    ...
    case AUTO_MODE:
        ...
        case 2://ÖÐ
						if(auto_mode_run_flag == 0){
							//Èç¹ûÊÇµÈ´ýÔËÐÐÄ£Ê½ÏÂ£¬°´ÏÂÖÐ¼ü¾Í¿ªÊ¼ÔËÐÐ
							printf("¿ªÊ¼Ñ­¼£\r\n");
							auto_mode_run_flag = 1;
							memcpy(run_str,"running...",sizeof("running..."));
						
						}else{
							//Èç¹ûÊÇÔËÐÐÄ£Ê½ÏÂ£¬°´ÏÂÖÐ¼ü¾ÍÍ£Ö¹ÔËÐÐ
			 ...
			 if(auto_mode_run_flag == 1)//ÊÇ·ñ¿ªÊ¼Ñ­¼££¿
				{
					//Ñ¹×¡×óÏßÔòÓÒ×ª
					//Ñ¹×¡ÓÒÏßÔò×ó×ª
					//¶¼²»Ñ¹ÔòÇ°½ø£¬Èç¹ûÕÏ°­ÎïÐ¡ÓÚÉè¶¨¾àÀëÔòÍ£Ö¹±¨¾¯
